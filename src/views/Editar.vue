...
<div class="form-group">
      ...
      <button @click="envia()" type="button">Salvar</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$http.get(`https://jsonbox.io/box_5bafe5f4bb577e7a9831/${this.$route.params.id}`)
      .then((res) => {
        this.cadastro = res.data;
      }).catch((res) => {
  ...
  methods: {
    ...
    envia() {
      this.$http.put(`https://jsonbox.io/box_5bafe5f4bb577e7a9831/${this.$route.params.id}`, this.cadastro)
        .then(() => {
